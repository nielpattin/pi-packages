...
Customizes the default [pi](https://github.com/badlogic/pi-mono) editor with a station-bar-style status bar, welcome overlay, and AI-generated "vibes" for loading messages. Inspired by [Powerlevel10k](https://github.com/romkatv/powerlevel10k) and [oh-my-pi](https://github.com/can1357/oh-my-pi).

## Features

**Editor stash** — Press `Alt+S` to save your editor content and clear the editor, type a quick prompt, and your stashed text auto-restores when the agent finishes. Toggles between stash, pop, and update-existing-stash. A `stash` indicator appears in the station bar while text is stashed.
...
**Context awareness** — Color-coded warnings at 70% (yellow) and 90% (red) context usage. During streaming, the context segment refreshes from live assistant usage instead of waiting for the next turn. Auto-compact indicator when enabled. If `pi-custom-compaction` is installed and enabled, the station bar automatically hides native context segments so the footer does not show stale post-summary usage.

**Token intelligence** — Smart formatting (1.2k, 45M), subscription detection (shows "(sub)" vs dollar cost).

**Sticky bash mode** — Toggle bash mode with `ctrl+b` or `/bash-mode`. It keeps a managed shell session alive for the current pi session, shows a dedicated `shell_mode` segment, and opens a full-screen transcript overlay with chat-style scroll, mouse, and key controls while `cd` or exported state persist across commands.
...
Activates automatically. Toggle with `/station`, fixed-editor mode with `/station fixed-editor on|off|toggle`, and wheel mode with `/station mouse-scroll on|off|toggle`.

Fixed editor is on by default.

- `/station fixed-editor off` — return to Pi’s regular scrolling layout
- `/station fixed-editor on` — re-enable the fixed editor
- `/station fixed-editor toggle` — switch between the two
...
The station bar uses one default layout. Segment options still control how individual segments render.

**Environment:** `STATION_BAR_NERD_FONTS=1` to force Nerd Fonts, `=0` for ASCII.

### Custom items from extension statuses

You can promote any extension status key into its own dedicated station bar item. This gives you a general way to register your own status items without changing this extension.

1. Any extension can publish status text through `ctx.ui.setStatus("my-key", "...value...")`.
2. Configure `station-bar.customItems` to place those keys on the left, right, or secondary row.
...
If you still prefer the old style, `"station-bar": "default"` continues to work.
...
The `stash` indicator appears in the station bar when `extension_statuses` is enabled. Active stash is still session-local and resets on session switch / disable, but stash history is persisted to `~/.pi/agent/station-bar/stash-history.json` so it survives restarts.
...
| ------------- | ------------------------------------ | ------------------------------------------------ |
| `basename`    | `station-bar`                        | Just the directory name (default)                |
| `abbreviated` | `…/extensions/station-bar`           | Full path with home abbreviated and length limit |
| `full`        | `~/.pi/agent/extensions/station-bar` | Complete path with home abbreviated              |

Configure via segment options: `path: { mode: "full" }`
...
Colors are configurable via pi's theme system. The default layout defines its own color scheme, and you can override individual colors and icons with a `theme.json` file in the extension directory.
...
Create `~/.pi/agent/extensions/station-bar/theme.json`:
