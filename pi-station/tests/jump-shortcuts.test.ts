...
const source = [
  readFileSync(new URL("../index.ts", import.meta.url), "utf-8"),
  readFileSync(new URL("../shortcut-manager/shortcut-manager.ts", import.meta.url), "utf-8"),
].join("\n");

const stationShortcutKeys = new Set([
  "stashHistory",
  "copyEditor",
  "cutEditor",
...
function stationDefaults(): Map<string, string> {
  const defaults = new Map<string, string>();
  for (const match of source.matchAll(/^  ([a-zA-Z0-9]+): "([^"]+)",?$/gm)) {
    const key = match[1];
    const value = match[2];
    if (key && value && stationShortcutKeys.has(key)) {
...
test("chat jump shortcuts are configurable and route through fixed editor scrolling", () => {
  ...
  assert.equal(defaults.get("toggleShortcut"), "ctrl+b");
...
test("thinking level changes invalidate station bar status rendering", () => {
...
test("extension status changes invalidate station bar status rendering", () => {
...
test("fixed editor captures Pi status messages with the editor cluster", () => {
  ...
  assert.match(source, /statusLines: \[\.\.\.aboveWidgetLines, \.\.\.renderStationStatusLines\(width\), \.\.\.statusContainerLines\]/);
...
test("station shortcut defaults do not claim reserved Pi shortcuts", () => {
  ...
  for (const [name, shortcut] of stationDefaults()) {
    if (name === "toggleShortcut") continue;
...
test("station fallback routing rejects reserved Pi shortcut defaults", () => {
...
test("station shortcuts have terminal-input fallback routing", () => {
  assert.match(source, /function getStationShortcutAction\(data: string\): StationShortcutAction \| null/);
  ...
  assert.match(source, /runStationShortcut\(ctx, stationShortcutAction\)/);
});
