...
import { resolveMutationTargetPath, writeFileAtomically } from "./core/fs-write.ts";
import {
   applyHashlineEdits,
...
import { formatHashlineReadPreview } from "./read-tool.ts";
...
function colorDiffLines(lines: string[], theme: { fg: (token: string, text: string) => string }): string[] {
...
function formatResultDiff(diff: string, theme: { fg: (token: string, text: string) => string }): string {
...
function isAppliedChangedResult(details: HashlineEditToolDetails | undefined): boolean {
   ...
   renderResult(result, { isPartial }, theme, context) {
      ...
      const markdown =
         context.lastComponent instanceof Markdown
            ...
            : new Markdown("", 0, 0, createRenderedEditMarkdownTheme(theme as any));
   ...
   async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      return withFileMutationQueue(mutationTargetPath, async () => {
         ...
         const successInput = {
            ...
            compatibilityDetails: compatibilityDetails as ResponseCompatibilityDetails | undefined,
            editsAttempted,
