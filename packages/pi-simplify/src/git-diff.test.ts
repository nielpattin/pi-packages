...
describe(getChangedFiles, () => {
   ...
   it("filters out deleted files", async () => {
      ...
      expect(files).toStrictEqual([{ path: "src/keep.ts", status: "modified" }]);
   });
   ...
   it("falls back to HEAD~1 when HEAD diff is empty", async () => {
      ...
      expect(files).toStrictEqual([{ path: "src/recent.ts", status: "modified" }]);
   });
   ...
   it("uses custom ref when provided", async () => {
      ...
      expect(files).toStrictEqual([{ path: "src/feature.ts", status: "added" }]);
   });
