...
function personalAgentPath(name: string): string {
   return join("/home/.pi/agents", `${name}.md`);
}
...
describe("AgentCreationWizard", () => {
   ...
   describe("generate wizard", () => {
      it("spawns agent and notifies on success when file is created", async () => {
         ...
         deps.manager.spawnAndWait.mockResolvedValue(createTestAgent({ status: "completed" }));
         ...
         const ui = makeMenuUI(["Project (.pi/agents/)", "Generate with Claude (recommended)"]);
         ui.input = vi
            .fn()
      ...
      it("notifies warning when spawn returns error status", async () => {
         const deps = makeDeps();
         deps.manager.spawnAndWait.mockResolvedValue(createTestAgent({ status: "error", error: "spawn failed" }));

         const ui = makeMenuUI(["Project (.pi/agents/)", "Generate with Claude (recommended)"]);
         ui.input = vi.fn().mockResolvedValueOnce("description").mockResolvedValueOnce("test-agent");

         const wizard = makeWizard(deps);
         await wizard.showCreateWizard(ui, STUB_SNAPSHOT);

         expect(ui.notify).toHaveBeenCalledWith("Generation failed: spawn failed", "warning");
      });
      ...
      it("prompts for overwrite when target file already exists", async () => {
         ...
         const ui = makeMenuUI(["Project (.pi/agents/)", "Generate with Claude (recommended)"]);
         ui.input = vi.fn().mockResolvedValueOnce("description").mockResolvedValueOnce("existing-agent");
   ...
   describe("manual wizard", () => {
      ...
      it("includes model line when a specific model is selected", async () => {
         const deps = makeDeps();
         const ui = makeMenuUI(["Project (.pi/agents/)", "Manual configuration", "all", "haiku", "inherit"]);
         ui.input = vi.fn().mockResolvedValueOnce("fast-agent").mockResolvedValueOnce("Fast agent");
      ...
      it("returns when user cancels name input", async () => {
         const deps = makeDeps();
         const ui = makeMenuUI(["Project (.pi/agents/)", "Manual configuration"]);
         ui.input = vi.fn().mockResolvedValueOnce(undefined);
