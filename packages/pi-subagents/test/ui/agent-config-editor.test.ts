...
import { AgentConfigEditor, buildEjectContent, buildMenuOptions } from "#src/ui/agent-config-editor";
...
function projectAgentPath(name: string): string {
   return join("/project/.pi/agents", `${name}.md`);
}
 ...
 */
function setupDetail(selectResults: (string | undefined)[], options: { filePath?: string; fileContent?: string } = {}) {
...
describe("createAgentConfigEditor", () => {
   describe("showAgentDetail", () => {
      it("notifies warning when agent type is not found", async () => {
         ...
         expect(ui.notify).toHaveBeenCalledWith('Agent config not found for "missing-agent".', "warning");
      });
      ...
      it("prompts for overwrite when ejected file already exists", async () => {
         ...
         expect(ui.confirm).toHaveBeenCalledWith("Overwrite", expect.stringContaining("already exists"));
         expect(fileOps.write).not.toHaveBeenCalled();
      });
      ...
      it("creates a disable-only file when no agent file exists", async () => {
         ...
         expect(fileOps.write).toHaveBeenCalledWith(projectAgentPath("test-agent"), "---\nenabled: false\n---\n");
      });
      ...
      it("enables agent by removing enabled:false from file", async () => {
         ...
         expect(fileOps.write).toHaveBeenCalledWith(filePath, "---\ndescription: test\n---\n\nprompt\n");
         expect(testRegistry.reload).toHaveBeenCalled();
      });
      ...
      it("removes empty override file when enabling", async () => {
         ...
         expect(fileOps.remove).toHaveBeenCalledWith(filePath);
         expect(ui.notify).toHaveBeenCalledWith(`Enabled test-agent (removed ${filePath})`, "info");
      });
   });
});
...
describe("buildMenuOptions", () => {
   it("shows Eject and Disable for a default agent with no file", () => {
      expect(buildMenuOptions({ isDefault: true }, undefined)).toEqual(["Eject (export as .md)", "Disable", "Back"]);
   });

   it("shows Edit, Disable, Reset, Delete for a default agent with override file", () => {
      expect(buildMenuOptions({ isDefault: true }, "/project/.pi/agents/test-agent.md")).toEqual([
         "Edit",
         "Disable",
         "Reset to default",
         "Delete",
   ...
   it("shows Edit, Disable, Delete for a custom agent with file", () => {
      expect(buildMenuOptions({ isDefault: false }, "/project/.pi/agents/test-agent.md")).toEqual([
         "Edit",
         "Disable",
         "Delete",
         "Back"
   ...
   it("shows Enable, Edit, Reset, Delete for a disabled default agent with file", () => {
      expect(buildMenuOptions({ isDefault: true, enabled: false }, "/project/.pi/agents/test-agent.md")).toEqual([
         "Enable",
         "Edit",
         "Reset to default",
         "Delete",
   ...
   it("shows Enable, Edit, Delete for a disabled custom agent with file", () => {
      expect(buildMenuOptions({ isDefault: false, enabled: false }, "/project/.pi/agents/test-agent.md")).toEqual([
         "Enable",
         "Edit",
         "Delete",
         "Back"
