...
describe("buildParentContext", () => {
   ...
   it("formats a user message with string content", () => {
      const result = buildParentContext(makeCtx([{ type: "message", message: { role: "user", content: "Hello" } }]));
   ...
   it("formats a compaction entry with a summary", () => {
      const result = buildParentContext(makeCtx([{ type: "compaction", summary: "Work done so far" }]));
   ...
   it("wraps non-empty output in the header/footer template", () => {
      const result = buildParentContext(makeCtx([{ type: "message", message: { role: "user", content: "test" } }]));
