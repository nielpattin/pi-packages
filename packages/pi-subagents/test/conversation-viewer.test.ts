 ...
 */
function assertRenderFitsWidths(messages: unknown[], widths = [40, 80, 120, 216], options?: TestViewerOptions): void {
...
describe("ConversationViewer", () => {
   describe("render width safety", () => {
      ...
      it("no line exceeds width with embedded ANSI escape codes in content", () => {
         ...
         assertRenderFitsWidths([{ role: "toolResult", toolUseId: "t1", content: [{ type: "text", text: ansiText }] }]);
      });

      it("no line exceeds width with long URLs", () => {
         const url = "https://example.com/" + "a/b/c/d/e/".repeat(30) + "?q=" + "x".repeat(100);
         assertRenderFitsWidths([{ role: "assistant", content: [{ type: "text", text: `Check this link: ${url}` }] }]);
      });
      ...
      it("no line exceeds width with wide table-like content", () => {
         ...
         assertRenderFitsWidths([{ role: "toolResult", toolUseId: "t1", content: [{ type: "text", text: table }] }]);
      });
      ...
      it("no line exceeds width with mixed ANSI + unicode content", () => {
         ...
         assertRenderFitsWidths([{ role: "toolResult", toolUseId: "t1", content: [{ type: "text", text }] }]);
      });
   });
