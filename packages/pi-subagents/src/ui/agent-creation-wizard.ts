...
export class AgentCreationWizard {
   ...
   async showCreateWizard(ui: MenuUI, parentSnapshot: ParentSnapshot): Promise<void> {
      ...
      const targetDir = location.startsWith("Project") ? this.projectAgentsDir : this.personalAgentsDir;

      const method = await ui.select("Creation method", ["Generate with Claude (recommended)", "Manual configuration"]);
      if (!method) return;
...
Write the file using the write tool. Only write the file, nothing else.`;
      ...
      if (this.fileOps.exists(targetPath)) {
         ...
         ui.notify("Agent generation completed but file was not created. Check the agent output.", "warning");
      }
   }
   ...
   private async showManualWizard(ui: MenuUI, targetDir: string): Promise<void> {
      ...
      if (toolChoice === "all") {
         ...
         const customTools = await ui.input("Tools (comma-separated)", BUILTIN_TOOL_NAMES.join(", "));
         if (!customTools) return;
         tools = customTools;
      }

      const modelChoice = await ui.select("Model", ["inherit (parent model)", "haiku", "sonnet", "opus", "custom..."]);
