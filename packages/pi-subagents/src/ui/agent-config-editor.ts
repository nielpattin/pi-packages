...
export function buildMenuOptions(cfg: { isDefault?: boolean; enabled?: boolean }, file: string | undefined): string[] {
...
export class AgentConfigEditor {
   ...
   async showAgentDetail(ui: MenuUI, name: string): Promise<void> {
      ...
      else if (choice === "Reset to default" && file) await this.handleReset(ui, name, file);
   ...
   private async handleDelete(ui: MenuUI, name: string, file: string): Promise<void> {
      const confirmed = await ui.confirm("Delete agent", `Delete ${name} (${file})?`);
      if (confirmed) {
   ...
   private async handleReset(ui: MenuUI, name: string, file: string): Promise<void> {
      const confirmed = await ui.confirm("Reset to default", `Delete override ${file} and restore embedded default?`);
      if (confirmed) {
         this.fileOps.remove(file);
         this.registry.reload();
