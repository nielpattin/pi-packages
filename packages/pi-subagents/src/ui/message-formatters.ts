 ...
 */
export function formatUserMessage(content: string | unknown[], width: number, ctx: FormatterContext): string[] | null {
   const { theme, wrapText } = ctx;
   const text = typeof content === "string" ? content : extractText(content);
   if (!text.trim()) return null;
   return [theme.fg("accent", "[User]"), ...wrapText(text.trim(), width)];
}
 ...
 */
export function formatBashExecution(msg: BashExecutionMessage, width: number, ctx: FormatterContext): string[] {
   const { theme, wrapText } = ctx;
