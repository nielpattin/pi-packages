...
export class AgentManager {
      ...
      return {
         ...
         onRunFinished: (agent) => {
            if (options.isBackground) {
               ...
               try {
                  this.observer?.onAgentCompleted(agent);
               } catch (err) {
                  debugLog("onAgentCompleted observer", err);
               }
            }
         },
   ...
   spawn(snapshot: ParentSnapshot, type: SubagentType, prompt: string, options: AgentSpawnConfig): string {
      ...
      const record = new Agent({
         ...
         // Shared deps
         runner: this.runner,
         worktree: options.isolation === "worktree" ? new WorktreeIsolation(this.worktrees, id) : undefined,
   ...
   dispose() {
      ...
      try {
         ...
         debugLog("pruneWorktrees on dispose", err);
      }
   }
}
