 ...
 *
 * # Why this is separate from Host's path
 *
 * Host synthesizes a single `tool` part on the latest assistant message
 * (`buildSyntheticTodoPart` in `core/hooks/magic-context/todo-view.ts`);
 * Host's wire serializer (`MessageV2.toModelMessagesEffect`) splits that
...
// path mapping into core for the rest of magic-context).
//
...
function hasToolCallWithId(message: PiAssistantMessage, callId: string): boolean {
...
function findToolResultAfter(messages: PiMessage[], assistantIndex: number, callId: string): number {
...
/**
 * Pi-shape adapter for the Host `SyntheticTodoPart`. Builds the
 ...
 */
function injectByAssistantId(messages: PiMessage[], messageId: string, part: SyntheticTodoPart): boolean {
 ...
 */
function injectIntoLatestAssistant(messages: PiMessage[], part: SyntheticTodoPart): string | null {
 ...
 * Host's `transform-postprocess-phase.ts` but uses Pi's wire-shape
 * helpers.
 *
...
export function injectSyntheticTodowriteForPi(args: {
   ...
   if (args.isCacheBusting) {
      ...
      if (
         ...
         // Mirrors the council Finding #1 v2 self-heal in the Host path.
      ...
      if (anchoredMessageId) {
         setPersistedTodoSyntheticAnchor(args.db, args.sessionId, part.callID, anchoredMessageId, args.lastTodoState);
   ...
   // behavior as Host's `injectToolPartIntoAssistantById`. Re-anchoring
