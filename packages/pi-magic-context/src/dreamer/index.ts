...
import { processDreamQueue, registerDreamProjectDirectory } from "#core/features/magic-context/dreamer";
import type { DreamRunResult } from "#core/features/magic-context/dreamer/runner";
...
type DreamTimerRegistration = Parameters<typeof defaultStartDreamScheduleTimer>[0];
...
interface ProjectRegistration {
   cleanup: () => void;
   /** Run one dream cycle for this project IMMEDIATELY (mirrors Host's
...
let startDreamScheduleTimerFn: typeof defaultStartDreamScheduleTimer = defaultStartDreamScheduleTimer;
...
export function registerPiDreamerProject(opts: PiDreamerOptions): void {
   ...
   // Pi parity for Host `command-handler.ts:236-246` (`/ctx-dream`):
   // after enqueueing, Host immediately drains the queue via
   ...
   const runOnce = async (): Promise<DreamRunResult | null> =>
      processDreamQueue({
         ...
         // the upstream SDK. The DreamTimerClient we pass
 ...
 * Host's `/ctx-dream` behavior. Returns the run result, or `null`
 ...
 * really there. Mirroring Host's behavior lets us actually drain
 * it on the same turn.
 */
export async function runPiDreamForProject(projectIdentity: string): Promise<DreamRunResult | null> {
   ...
   const session = {
      ...
      prompt: async (args: SessionPromptArgs) => {
         ...
         try {
            ...
            if (!result.ok) {
               throw new Error(`Pi dreamer subagent failed (${result.reason}): ${result.error}`);
            }
...
function normalizeFallbackModels(value: DreamerConfig["fallback_models"]): readonly string[] | undefined {
...
function makeMessage(role: "user" | "assistant", parts: Array<{ type: "text"; text: string }>): unknown {
   return {
      info: {
         role,
