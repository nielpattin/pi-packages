 ...
 *   single-pass-per-LLM-call, so Host-style cache-bust avoidance
 *   doesn't apply. If a Pi provider exposes prompt cache later we'd
 *   add the relevant subset.
 * - Compaction marker injection. Host-only — Pi doesn't have a
 ...
 * as the Host `messages-transform` wrapper.
 */
...
import { applyFlushedStatuses, applyPendingOperations } from "#core/hooks/magic-context/apply-operations";
import { applyMidTurnDeferral, detectMidTurnBypassReason } from "#core/hooks/magic-context/boundary-execution";
...
import { resolveContextLimit, resolveExecuteThreshold } from "#core/hooks/magic-context/event-resolvers";
...
/** Force-materialization threshold — mirrors Host's FORCE_MATERIALIZE_PERCENTAGE (85%). */
...
/** Emergency-block threshold — mirrors Host's >=95% emergency path. */
 ...
 * config specifies one. Matches Host's schema default
 * (`core/config/schema/magic-context.ts:303` → `.default(50)`).
 */
const DEFAULT_CLEAR_REASONING_AGE = 50;

/**
 * Per-session emergency-notification dedup. Mirrors Host's
 ...
 * note-nudge `commit_detected` trigger. Mirrors Host's
...
/**
 * Three independent per-session refresh signals — mirrors Host's
...
export function trackSessionForProject(projectIdentity: string, sessionId: string): void {
...
function updateSessionProjectTracking(sessionId: string, projectIdentity: string): void {
...
export function signalPiSystemPromptRefreshForProject(projectIdentity: string): void {
...
export function getPiToolUsageSinceUserTurnForTest(sessionId: string): number | undefined {
   return toolUsageSinceUserTurn.get(sessionId);
}

function onPiNewUserMessage(args: { db: ContextDatabase; sessionId: string }): void {
   ...
   if (
      ...
      setPersistedStickyTurnReminder(args.db, args.sessionId, TOOL_HEAVY_TURN_REMINDER_TEXT);
   }
...
function persistLastTransformErrorIfChanged(db: ContextDatabase, sessionId: string, summary: string): void {
   try {
      ...
      sessionLog(sessionId, `transform error persistence failed: ${err instanceof Error ? err.message : String(err)}`);
   }
}

function clearLastTransformErrorIfSet(db: ContextDatabase, sessionId: string): void {
   try {
      const current = getOrCreateSessionMeta(db, sessionId).lastTransformError;
      if (current !== null) {
      ...
      sessionLog(sessionId, `transform error clear failed: ${err instanceof Error ? err.message : String(err)}`);
   }
}
...
export interface PiHistorianOptions {
    ...
    *  Host's `historian.two_pass` config. */
   twoPass?: boolean;
    ...
    * pressure-driven trigger points. Mirrors Host's
   ...
   /** Token-based execute-threshold overrides. Mirrors Host `execute_threshold_tokens`. */
   executeThresholdTokens?: {
      default?: number;
      [modelKey: string]: number | undefined;
   };
   /** Commit-cluster trigger config. Mirrors Host `commit_cluster_trigger`. */
...
export interface PiNudgeOptions {
   /** Number of most-recent tags treated as protected (mirrors Host `protected_tags`). */
   protectedTags: number;
   /** Base interval between rolling reminders, in tokens (mirrors Host `nudge_interval_tokens`). */
 ...
 * are computed against the same local Magic Context DB Host uses.
 */
...
export interface PiHeuristicsOptions {
    ...
    * cleared on cache-busting passes. Mirrors Host's
    * `clear_reasoning_age` config (`core/config/schema/magic-context.ts:303`).
    * Default `50` matches Host. Pi previously hardcoded `30`, which
...
export interface PiContextHandlerOptions {
    ...
    * they can't act on. Mirrors Host behavior.
    */
   ...
   /**
    * Number of most-recent tags treated as protected (mirrors Host
    ...
    * the local Magic Context DB, so memories ARE cross-harness.
    */
...
export function collectMessageEntryIdsByRef(
   ...
   for (let i = 0; i < messages.length; i++) {
      ...
      const fingerprintBucket = fingerprint ? entryIdsByFingerprint.get(fingerprint) : undefined;
      ...
      const fingerprintId =
         fingerprintBucket?.length === 1 && !consumedFingerprintIds.has(fingerprintBucket[0] as string)
            ? fingerprintBucket[0]
            : undefined;
...
function readPiBranchEntriesForContext(ctx: ExtensionContext, sessionId: string): readonly unknown[] | null {
   const sm = ctx.sessionManager as { getBranch?: (fromId?: string) => unknown[] } | undefined;
   if (typeof sm?.getBranch !== "function") return null;
   try {
      const entries = sm.getBranch.call(sm);
      return Array.isArray(entries) ? entries : null;
   } catch (error) {
      sessionLog(sessionId, `Pi branch pre-read failed: ${error instanceof Error ? error.message : String(error)}`);
      return null;
   }
}
 ...
 * Host plugin's tagger. It maintains in-memory state (the
 * monotonic counter, assignment map) across `context` events so tag
 * numbers stay stable for the duration of the Pi session.
 */
export function registerPiContextHandler(pi: ExtensionAPI, options: PiContextHandlerOptions): void {
   ...
   pi.on("context", async (event, ctx) => {
      ...
      try {
         ...
         if (sessionId === undefined) {
            ...
            log("[magic-context][pi] context event fired with no session id (falling through unmodified)");
            return;
         }
         ...
         const isFirstContextPassForSession = !firstContextPassSeenBySession.has(sessionId);
         ...
         const modelChanged =
            previousModelKey !== undefined && currentModelKey !== undefined && previousModelKey !== currentModelKey;
         ...
         if (
            ...
            (sessionMetaForUsage.lastContextPercentage > 0 || sessionMetaForUsage.lastInputTokens > 0)
         ) {
         ...
         if (sessionMetaForUsage.lastContextPercentage > 0 && sessionMetaForUsage.lastInputTokens > 0) {
            ...
            usagePercentage = typeof piUsage?.percent === "number" ? piUsage.percent : 0;
            usageInputTokens = typeof piUsage?.tokens === "number" ? piUsage.tokens : 0;
         }
         let usageContextLimit =
            typeof piUsage?.contextWindow === "number" && piUsage.contextWindow > 0 ? piUsage.contextWindow : undefined;
         ...
         //
         // Mirrors Host's transform.ts:401 wiring exactly. The
         ...
         const piMessageCount = Array.isArray(event.messages) ? event.messages.length : 0;
         ...
         const deferredExecutePending = peekDeferredExecutePending(options.db, sessionId);
         if (deferredExecutePending !== null && schedulerDecision === "defer" && !midTurn) {
            schedulerDecision = "execute";
         }
         sessionLog(
            sessionId,
         ...
         // Mirrors Host transform-postprocess-phase.ts:145-146.
         const forceMaterialization = usagePercentage >= FORCE_MATERIALIZATION_PERCENTAGE;
         ...
         // possible before the LLM call. Mirrors Host's >=95%
         // emergency path in transform.ts (~line 514+).
         //
         // Pi differences vs Host:
         ...
         if (isEmergency) {
            const lastNotifiedAt = lastEmergencyNotificationAtMs.get(sessionId) ?? 0;
            const now = Date.now();
         ...
         // signal — to mirror Host's transform.ts:444 exactly.
         //
         ...
         if (nudgerFn && options.nudge) {
            try {
               ...
               sessionLog(sessionId, `rolling nudge failed: ${err instanceof Error ? err.message : String(err)}`);
            }
         }
         ...
         if (options.autoSearch?.enabled) {
            try {
               await ensureProjectRegisteredFromPiDirectory(projectDirectory, options.db);
         ...
         // Synthetic todowrite injection — Pi parity with Host's
         ...
         //
         // Cache-busting gate parity: Host uses
         ...
         // Host's broader "execute pass that actually mutated state"
         // semantics.
         //
         // Subagents skip — they don't get synthetic injection in
         // Host either (see B7 `args.fullFeatureMode` gate).
         try {
            const sessionMetaForTodo = getOrCreateSessionMeta(options.db, sessionId);
            if (!sessionMetaForTodo.isSubagent && sessionMetaForTodo.lastTodoState !== "") {
         ...
         try {
            ...
            setSessionWorkMetrics(options.db, sessionId, metrics.newWorkTokens, metrics.totalInputTokens);
         } catch (err) {
            sessionLog(sessionId, `work-metrics update failed: ${err instanceof Error ? err.message : String(err)}`);
         }
         ...
         log(`[magic-context][pi] context handler failed (continuing without mutation): ${message}`, stack);
         if (sessionIdForError) {
            persistLastTransformErrorIfChanged(options.db, sessionIdForError, summarizeTransformError(err));
         }
   ...
   log("[magic-context][pi] registered context handler (tagging + drops + nudges)");
}
...
export function resolvePiHistorianTriggerInputs(args: {
   ...
   return {
      ...
      triggerBudget: deriveTriggerBudget(contextLimit, executeThresholdPercentage),
      ...
      try {
         await runPiHistorian({
            ...
            onPublished: () => {
               ...
               // compartments. Mirrors Host's onInjectionCacheCleared
...
function maybeFireHistorian(args: {
   ...
   // Prefer Host-equivalent pressure persisted by message_end.
   ...
   try {
      if (isFirstContextPassForSession) {
         ...
         if (sessionMeta.compartmentInProgress && !inFlightHistorian.has(sessionId)) {
...
interface RunPipelineArgs {
   ...
   ctxReduceEnabled: boolean;
   protectedTags: number;
   /** Heuristic-cleanup config — when omitted, defaults to Host parity values. */
   heuristics?: {
      autoDropToolAge: number;
      dropToolStructure: boolean;
      caveman?: { enabled: boolean; minChars: number };
    ...
    * cached injection). Mirrors Host's `schedulerDecisionEarly`.
    */
   schedulerDecision: "execute" | "defer";
   /**
    * Force-materialization signal: when true, drop-all-tools mode
    * activates (mirrors Host's >=85% emergency cleanup). Caller
    ...
    * the prepared block rebuilt on this pass. Mirrors Host's
    ...
    * defer passes replay the cleared state. Mirrors Host's
    * `clearOldReasoning` + `replayClearedReasoning` pair.
    *
    * Host PR #24146 (preserve empty reasoning_content for DeepSeek
    ...
    * messages with large gaps. Mirrors Host's
...
interface RunPipelineResult {
   ...
   /** Aggregate counts for log parity with Host. */
...
async function runPipeline(args: RunPipelineArgs): Promise<RunPipelineResult> {
   ...
   // and skipped. Same invariants as Host's `injectTemporalMarkers`
   ...
   if (args.temporalAwareness) {
      ...
      try {
         ...
         if (injected > 0) {
            sessionLog(args.sessionId, `temporal-awareness: injected ${injected} gap markers`);
         }
      } catch (err) {
         sessionLog(
            args.sessionId,
   ...
   const currentTurnId = (() => {
      const ids = buildPiMessageIdByIndex(args.messages as PiAgentMessage[], args.entryIds ?? null);
      return findLatestUserMessageIdPi(args.messages as PiAgentMessage[], ids)?.messageId ?? null;
   })();
   ...
   // Host does, because they do not share the primary agent's turn cache.
   ...
   // 1b. Note-nudge `commit_detected` trigger. Mirrors Host's logic
   ...
   // Mirrors Host's transform-postprocess-phase.ts:184-186 gating:
   ...
   const deferredExecuteWasPending = peekDeferredExecutePending(args.db, args.sessionId) !== null;
   ...
   const canConsumeDeferredLate = baseShouldApplyPendingOps || shouldRunHeuristics;
   ...
   if (shouldApplyPendingOps) {
      ...
      try {
         ...
         sessionLog(args.sessionId, `pending operations failed: ${err instanceof Error ? err.message : String(err)}`);
         throw err;
      }
   } else {
      sessionLog(
   ...
   // execute passes. Mirrors Host's `applyFlushedStatuses` call
   // at transform.ts:728.
   //
   ...
   // the full-table scan we eliminated in Host's transform.
   ...
   // provider prompt cache. Mirrors Host's
   ...
   if (args.reasoningClearing) {
      try {
         ...
         logTransformTiming(args.sessionId, "replayReasoningClearing", tReplayReasoning);
         logTransformTiming(
            args.sessionId,
   ...
   //
   // Mirrors Host's `replayCavemanCompression` call in
   ...
   //
   // - stripStructuralNoise: removes Host AI-SDK-specific part
   ...
   // Mirrors Host's `applyHeuristicCleanup` call in
   ...
   // Host's `clearOldReasoning` (strip-content.ts) gated to
   // execute passes via the same scheduler decision used for
   // heuristic cleanup.
   if (args.reasoningClearing && (args.schedulerDecision === "execute" || args.forceMaterialization === true)) {
      try {
   ...
   // block. Mirrors Host's prepareCompartmentInjection +
   ...
   if (args.injection) {
      try {
         ...
         logTransformTiming(args.sessionId, "prepareCompartmentInjection", tInjection);
   ...
   if (executedWorkThisPass) {
      try {
         ...
         sessionLog(args.sessionId, `[boundary-exec] drain failed (continuing): ${String(err)}`);
      }
   }
   ...
   // receives. Mirrors Host's transform.ts:996-1127. Best-effort —
...
/**
 * Apply the rolling/iteration nudge after tagging. Mirrors Host's
 ...
 * pass behaves like an Host "cache-busting" pass: nudges always
...
function applyStickyTurnReminder(args: {
   ...
   const messageIdByIndex = buildPiMessageIdByIndex(args.messages, args.entryIds);
   ...
   if (args.entryIds === null) {
      sessionLog(args.sessionId, "Pi sticky turn reminder: strict resolution failed; replay-only until next pass");
      return args.messages;
   }
   ...
   if (latest) {
      ...
      setPersistedStickyTurnReminder(args.db, args.sessionId, reminder.text, latest.messageId);
   }
   return args.messages;
}

/**
 * Apply note-nudge replay + delivery. Mirrors Host's
...
function applyNoteNudges(args: {
   ...
   for (const anchor of getNoteNudgeAnchors(db, sessionId)) {
      appendReminderToUserMessageByIdPi(messages, replayMessageIdByIndex, anchor.messageId, anchor.text);
   }
   for (const decision of getAutoSearchHintDecisions(db, sessionId)) {
      if (decision.decision === "hint") {
         appendReminderToUserMessageByIdPi(messages, replayMessageIdByIndex, decision.messageId, decision.text);
      }
   }
   ...
   // Host's transform-postprocess-phase.ts:647 wiring.
   ...
   const deferredNoteText = peekNoteNudgeText(db, sessionId, latestUserId, projectIdentity, noteReadStillVisible);
   if (deferredNoteText) {
      if (entryIds === null) {
         sessionLog(sessionId, "Pi note-nudge: strict resolution failed; deferring delivery to next pass");
         return messages;
      }
      ...
      const outcome = markNoteNudgeDelivered(db, sessionId, noteInstruction, anchoredId);
      if (latestUser && outcome.ok) {
         appendReminderToPiUserMessage(messages[latestUser.index] as PiAgentMessage, noteInstruction);
      } else if (!outcome.ok) {
         sessionLog(sessionId, `Pi note-nudge delivery skipped wire append: ${outcome.kind}`);
      }
   }

   if (entryIds !== null && !entryIds.includes(undefined)) {
      const visibleIds = new Set(entryIds.filter((id): id is string => typeof id === "string"));
 ...
 * `core/hooks/magic-context/transform-message-helpers.ts`,
 ...
 * `appendReminderToUserMessageById` from Host's
 ...
 */
function appendReminderToPiUserMessage(message: PiAgentMessage, reminder: string): void {
 ...
 *
 * Counterpart to Host `session.deleted` cleanup in
