/**
 * Pi compressor runner — parity with Host's
 * `compartment-runner-compressor.ts`.
 *
 * Host runs a second-stage historian when the rendered
 ...
 * `PiSubagentRunner`) rather than Host's child-session SDK. The
...
import { parseCompartmentOutput } from "#core/hooks/magic-context/compartment-parser";
import { buildCompressorPrompt, COMPARTMENT_AGENT_SYSTEM_PROMPT } from "#core/hooks/magic-context/compartment-prompt";
...
function scoreCompartments(db: ContextDatabase, sessionId: string, compartments: Compartment[]): ScoredCompartment[] {
...
/**
 * Exported for tests. Same selection contract as Host: try depth
...
export function selectPiCompressionBand(
   ...
   for (const targetDepth of [...tiers].sort((a, b) => a - b)) {
      ...
      while (i < scanEnd) {
         ...
         if (!anchor || anchor.averageDepth >= maxMergeDepth || Math.round(anchor.averageDepth) !== targetDepth) {
            i++;
            continue;
         }
...
export async function runPiCompressionPassIfNeeded(deps: PiCompressorDeps): Promise<boolean> {
   const {
      db,
      sessionId,
      historyBudgetTokens,
   ...
   const avgDepth = band.reduce((sum, s) => sum + s.averageDepth, 0) / band.length;
   ...
   const outputCount = mergeRatio > 0 ? Math.max(1, Math.ceil(band.length / mergeRatio)) : 1;
...
async function runCompressorPass(
   ...
   try {
      ...
      if (parsed.compartments.length === 0) {
         sessionLog(args.sessionId, "compressor: historian returned no compartments");
         return null;
      }

      const snapped = snapLLMOutputToInputBoundaries(parsed.compartments, args.compartments);
      if (!snapped) {
         sessionLog(args.sessionId, "compressor: rejecting — LLM output contains ordinal(s) outside input range");
         return null;
      }
...
function finalizeCompression(args: {
   ...
   const { db, sessionId, compartments, compressed, originalStart, originalEnd } = args;
...
export function isPiCompressorOnCooldown(sessionId: string, cooldownMs: number): boolean {
