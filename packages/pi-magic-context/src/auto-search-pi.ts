 ...
 *
 * This is the Pi-shaped counterpart to Host's
 ...
 * turn. We mirror Host's per-session cache (Host lines 33-38,
 ...
 * the same 3000ms cap as Host (lines 40-47, 222-229, 239-246). On
 ...
 * `<ctx-search-hint>`, or `<ctx-search-auto>`, matching Host's stacked
...
import type { UnifiedSearchOptions, UnifiedSearchResult } from "#core/features/magic-context/search";
...
export async function runAutoSearchHintForPi(args: {
   ...
   if (strictResolutionFailed) {
      sessionLog(sessionId, "Pi auto-search: strict entry-id resolution failed; replayed persisted decisions only");
      return messages;
   }
   ...
   // Suppression check runs on raw text before stripping; Host does the
   ...
   if (results[0].score < scoreThreshold) {
      sessionLog(sessionId, `auto-search: top score ${results[0].score.toFixed(3)} below threshold ${scoreThreshold}`);
