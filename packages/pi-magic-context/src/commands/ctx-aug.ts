 ...
 *
 * Mirrors the Host `/ctx-aug` flow (see
 * core/hooks/magic-context/command-handler.ts#executeAugmentation):
 ...
 *      triggers a turn.
 *
 * Implementation differences from Host:
 * - Host uses `client.session.create() + client.session.prompt()` to spawn
 ...
 * - Host commits the augmented prompt via a server-side `client.session
 ...
 * - Host bubbles sidekick failures back through the command handler. Pi
 ...
 * - Host displays the "preparing" message as an ignored notification.
 ...
 *
 * Cache safety note: This is the same design we use in Host — the
 ...
 *
 * Same shape as Host's `SidekickConfig` minus the Host-specific
 ...
 * will print a "not configured" message to the user, matching Host's
 * behavior of surfacing the missing configuration via notification rather
 * than hiding the command entirely.
 */
export function registerCtxAugCommand(pi: ExtensionAPI, config: PiSidekickConfig | undefined): void {
   ...
   pi.registerCommand("ctx-aug", {
      ...
      handler: async (args, ctx) => {
         ...
         sessionLog(sessionLabel, `/ctx-aug: sidekick returned ${sidekickText.length} chars in ${result.durationMs}ms`);

         // If sidekick returned the literal "no relevant memories" sentinel
         // (or near-empty text), skip the augmentation block entirely —
         // the agent gets a cleaner prompt. This matches Host's
