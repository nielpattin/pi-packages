...
import { enqueueDream } from "#core/features/magic-context/dreamer/queue";
...
export function registerCtxDreamCommand(
   ...
   pi.registerCommand("ctx-dream", {
      ...
      handler: async (_args, ctx) => {
         ...
         const enqueued = enqueueDream(deps.db, project.projectIdentity, "manual", true);
         if (!enqueued) {
            // Already queued or actively running. Mirrors Host's
         ...
         try {
            ...
            let summary: string;
            if (!result) {
               summary = "Dream queued, but another worker is already processing the queue.";
            } else if (result.tasks.length === 0) {
               summary = "Dreamer is configured, but no dream tasks are enabled for this project.";
            } else {
            ...
            sessionLog(project.projectIdentity, `/ctx-dream failed to drain queue: ${message}`);
            sendCtxStatusMessage(
               pi,
               {
                  title: "/ctx-dream",
