 ...
 *
 * Mirrors `compartment-runner-incremental.ts` (Host) but uses
 ...
 *   - Host-style compaction markers (Pi has native compaction)
 ...
 *
 * Failure handling philosophy: like Host, this runner is fail-closed —
 ...
 *
 * Logs go through the shared sessionLog so Host log-tailing tools
...
import type { SubagentProgressEvent, SubagentRunner, SubagentRunResult } from "#core/shared/subagent-runner";
...
export interface PiHistorianDeps {
   /** SQLite handle for the local Magic Context DB. */
   db: Database;
    ...
    *  Host's `historian.two_pass` config. Editor validation falls back
   ...
   try {
      ...
      await withRawMessageProvider(sessionId, provider, async () => {
         ...
         if (existingValidationError) {
            ...
            await notify(`Historian skipped: existing stored compartments are invalid: ${existingValidationError}`);
            return;
         }
         ...
         // memories written by Host show up in this Pi historian run.
         ...
         // <project>/.host/magic-context/historian/ so it stays
         // inside the project boundary on the Host side and remains a
         ...
         // Host parity in `compartment-runner-historian.ts`,
         ...
         // Repair retry on validation failure (mirrors Host behavior).
         ...
         // Optional two-pass editor refinement. Mirrors Host's
         ...
         // the next user turn. Mirrors Host's
         ...
         // show up alongside facts written by Host historian.
         if (memoryEnabled !== false && autoPromote !== false) {
            promoteSessionFactsToMemory(db, sessionId, projectPath, validatedPass.facts ?? []);
         }
...
/** Internal validation result classification — mirrors Host pass result shape. */
 ...
 * (deferred compaction marker issue
 * e2e test).
 *
