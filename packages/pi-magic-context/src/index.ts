 ...
 *
 * Loaded once per Pi session through package.json. Boots the local SQLite
 * store and registers tools, command handlers, system-prompt injection,
 * historian scheduling, dreamer scheduling, and session cleanup.
 */

import { createRequire } from "node:module";
import { join } from "node:path";
import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";
import { isDreamerRunnable } from "#core/config/agent-disable";
import type {
   DreamerConfig,
   HistorianConfig,
   MagicContextConfig,
...
import { deriveHistorianChunkTokens, resolveHistorianContextLimit } from "#core/hooks/magic-context/derive-budgets";
import { resolveCacheTtl } from "#core/hooks/magic-context/event-resolvers";
import { clearNoteNudgeTriggerAndCooldown, onNoteTrigger } from "#core/hooks/magic-context/note-nudger";
...
import { clearModelsDevCache, getModelsDevContextLimit } from "#core/shared/models-dev-cache";
...
import { buildMagicContextBlock, clearPiSystemPromptSession, processSystemPromptForCache } from "./system-prompt";
...
export async function persistPiPressureFromMessageEnd(args: {
   ...
   const msg =
      args.message && typeof args.message === "object" ? (args.message as { errorMessage?: unknown }) : undefined;
...
export function resolveSidekickFromConfig(config: MagicContextConfig): PiSidekickConfig | undefined {
...
export function resolveHistorianFromConfig(config: MagicContextConfig): PiHistorianOptions | undefined {
   ...
   // session model + effective execute threshold to match Host.
   ...
   return {
      ...
      // duplicates. Mirrors Host's config flag — defaults to false
...
function resolveNudgeFromConfig(config: MagicContextConfig): PiNudgeOptions | undefined {
...
export default async function (pi: ExtensionAPI): Promise<void> {
   ...
   // nothing — same fail-closed posture the Host plugin uses.
   if (!config.enabled) {
   ...
   // the Host plugin uses (insertMemory, unifiedSearch, addNote, …)
   // via the local Magic Context DB. Cross-harness memory sharing is automatic
   ...
   registerMagicContextTools(pi, {
      ...
      // Match Host's gating: when ctx_reduce_enabled is false,
   ...
   const sidekickConfig: PiSidekickConfig | undefined = resolveSidekickFromConfig(config);
   ...
   // reaches command-surface parity with the Host plugin. All four
   // commands emit `pi.sendMessage(..., { triggerTurn: false })` — they
   // are never visible to the LLM and never trigger a turn. They mirror
   // the behavior of Host's command-handler.ts but use Pi-native
   // surfaces (registerCommand + sendMessage) instead of Host's
   ...
   //
   // Pi has prefix caching the same way Host does — every major
   ...
   // next call. The protections here mirror Host's
   // `experimental.chat.system.transform` handler in
   // `system-prompt-hash.ts`.
   pi.on("before_agent_start", async (event, ctx) => {
      // Startup release announcement (Pi parity with Host TUI dialog +
      // Desktop ignored message). Fires once per ANNOUNCEMENT_VERSION across
      // the whole machine — persistence file is shared with the Host
      ...
      try {
         if (ctx.hasUI && shouldShowAnnouncement()) {
            ...
            const sections = [`✨ Magic Context v${ANNOUNCEMENT_VERSION} — what's new:`, "", featureText];
      ...
      try {
         ...
         const block = buildMagicContextBlock({
            ...
            sessionId,
            memoryEnabled: config.memory.enabled,
            injectDocs: isDreamerRunnable(config) && (config.dreamer?.inject_docs ?? true),
            includeGuidance: true,
            protectedTags: config.protected_tags,
            ctxReduceEnabled: config.ctx_reduce_enabled,
            dreamerEnabled: isDreamerRunnable(config),
            dropToolStructure: config.drop_tool_structure,
            temporalAwarenessEnabled: config.experimental?.temporal_awareness ?? false,
            cavemanTextCompressionEnabled:
               config.ctx_reduce_enabled === false && config.experimental?.caveman_text_compression?.enabled === true,
   ...
   // agent tool usage. Mirrors Host's `tool.execute.after` hook in
   ...
   // Host's `tool.execute.before`/`after` that have full args
   ...
   //     so we still skip the trigger for them. Mirrors Host's
   ...
   pi.on("tool_execution_start", async (event, ctx) => {
      try {
         ...
         if (event.toolName === "todowrite") {
            ...
            // Host hook-handlers.ts:386-401). Persist normalized
   ...
   // Host's `experimental.text.complete` handler which scrubs the
   ...
   // the persistence call — same effect as Host's hook on a
   // different harness.
   //
   ...
   pi.on("message_end", async (event, ctx) => {
      ...
      // defer (no usage data) — neither matches Host parity.
      try {
         ...
         if (endedMsg?.role === "assistant" && typeof endedMsg.id === "string" && endedMsg.id.length > 0) {
         ...
         //   Pi could execute it locally, matching what Host
         ...
         // injection downstream (mirrors Host `fullFeatureMode`
         // gate).
         try {
            ...
            if (!sessionMetaForTodo.isSubagent) {
               ...
               if (msg && msg.role === "assistant" && Array.isArray(msg.content)) {
                  for (const block of msg.content) {
                     ...
                     const args = b.arguments as { todos?: unknown } | null | undefined;
   ...
   // Host's `session.deleted` handler in `event-handler.ts`.
   pi.on("session_before_switch", (_event, ctx) => {
      try {
         ...
         const outgoingSessionId = typeof sm?.getSessionId === "function" ? sm.getSessionId() : undefined;
