...
function resolveBundledPiCli(): string | null {
   ...
   try {
      ...
      const cliPath = join(agentDir, "node_modules", "@earendil-works", "pi-coding-agent", "dist", "cli.js");
      if (existsSync(cliPath)) return cliPath;
   } catch {}
...
function resolveSubagentEntryPath(): string | undefined {
   try {
      ...
      // Dev fallback: when running source from the flattened src/ package
 ...
 * action surface (update/merge/archive). Mirrors Host's
 ...
 *   API equivalent to Host's `client.session.create() / .prompt()`.
...
export class PiSubagentRunner implements SubagentRunner {
    ...
    *      npm-only installs, and package users.
...
`,
            ...
            if (e.type === "message_end" && e.message) {
               ...
               if (m.role === "assistant") {
                  ...
                  const isTerminalStopReason =
                     ...
                     (m.stopReason === "stop" || m.stopReason === "length" || m.stopReason === "aborted");
         ...
         child.on("close", (code, signal) => {
            ...
            if (sawAgentEnd) {
               ...
               if (finalStopReason === "error" || finalStopReason === "aborted" || finalStopReason === "length") {
                  settle({
                     ok: false,
...
function isFallbackEligible(reason: string): boolean {
   return (
      reason === "model_failed" || reason === "truncated" || reason === "non_zero_exit" || reason === "no_assistant"
   );
}
...
export function buildArgs(options: SubagentRunOptions): string[] {
   const args: string[] = [
      ...
      // directly to Host's "hidden subagent" pattern, which lets
   ...
   const shouldLoadSubagentExtension =
      SUBAGENT_ENTRY_PATH && (options.agent === "sidekick" || DREAMER_ACTION_AGENTS.has(options.agent));
   if (shouldLoadSubagentExtension) {
      ...
      // surface (update/merge/archive). Mirrors Host's
   ...
   // Prompt is piped via stdin to avoid ENAMETOOLONG on Windows.
...
export function parsePiEventLine(line: string): { ok: true; event: unknown } | { ok: false; error: string } {
   try {
