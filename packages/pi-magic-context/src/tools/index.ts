 ...
 * shipped. This matches Host's gating in `tool-registry.ts`.
 */
...
export interface RegisterToolsOptions {
    ...
    *  (./index.ts) leaves this false to match Host's primary-agent surface. */
...
export function registerMagicContextTools(pi: ExtensionAPI, opts: RegisterToolsOptions): void {
   ...
   // `todowrite` parity with Host. Pi-coding-agent has no built-in
   ...
   //     transcript-pi.ts, matching Host's transform.ts gate
