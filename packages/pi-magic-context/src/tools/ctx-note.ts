 ...
 *
 * Action surface mirrors Host's `core/tools/ctx-note/tools.ts`:
...
export function createCtxNoteTool(deps: CtxNoteToolDeps): ToolDefinition<typeof ParamsSchema> {
   return {
      ...
      async execute(_toolCallId, params: CtxNoteParams, _signal, _onUpdate, ctx) {
         ...
         const action = params.action ?? (typeof params.content === "string" ? "write" : "read");
         ...
         if (action === "dismiss") {
            ...
            return dismissed
               ...
               : err(`Error: Note #${params.note_id} not found in your session/project or already dismissed.`);
         }
         ...
         // mixed-view marker (matches Host parity). Coercing to
 ...
 * the requested filter. The DEFAULT (filter undefined) matches Host's
 ...
 * Host parity (see core/tools/ctx-note/tools.ts:46-95).
 *
