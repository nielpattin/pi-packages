 ...
 *
 * Action surface mirrors Host's `core/tools/ctx-memory/tools.ts`.
 ...
 *
 * Allowlist gating mirrors Host's `allowedActions` deps field. In Host,
 ...
 * in the local Magic Context DB, so a memory written from the pi-plugin is
 * immediately visible to Host sessions on the same project (and vice
 * versa).
 */

import type { ToolDefinition } from "@earendil-works/pi-coding-agent";
import { invalidateAllMemoryBlockCaches } from "#core/features/magic-context/compartment-storage";
import {
   archiveMemory,
   CATEGORY_PRIORITY,
...
const ALL_ACTIONS = ["write", "delete", "list", "update", "merge", "archive"] as const;
type CtxMemoryAction = (typeof ALL_ACTIONS)[number];

const DREAMER_ONLY_ACTIONS: ReadonlySet<CtxMemoryAction> = new Set(["list", "update", "merge", "archive"]);
...
function queueEmbedding(args: { deps: CtxMemoryToolDeps; projectIdentity: string; memoryId: number; content: string }) {
   ...
   void (async () => {
      try {
         ...
         if (!result) {
            log(`[magic-context-pi] embedding skipped for memory ${args.memoryId}: provider unavailable.`);
            return;
         }
...
export function createCtxMemoryTool(deps: CtxMemoryToolDeps): ToolDefinition<typeof ParamsSchema> {
   ...
   const description = dreamerAllowed
      ? "Manage cross-session project memories. Memories persist across sessions and are " +
        "shared with Host sessions on the same project. " +
   ...
   return {
      ...
      async execute(_toolCallId, params: CtxMemoryParams, _signal, _onUpdate, ctx) {
         ...
         // Host's `if (toolContext.agent !== DREAMER_AGENT && !allowedActions.includes(args.action))`.
         if (!dreamerAllowed && DREAMER_ONLY_ACTIONS.has(params.action)) {
            return err(`Error: Action '${String(params.action)}' is not allowed in this context.`);
         }
         ...
         if (params.action === "write") {
            ...
            if (!isMemoryCategory(rawCategory)) {
               return err(`Error: Unknown memory category '${rawCategory}'. Valid: ${CATEGORY_PRIORITY.join(", ")}`);
            }
            ...
            if (existing) {
               ...
               return ok(`Memory already exists [ID: ${existing.id}] in ${rawCategory} (seen count incremented).`);
            }
         ...
         if (params.action === "merge") {
            const ids = params.ids?.filter((id): id is number => Number.isInteger(id));
            if (!ids || ids.length < 2) {
               return err("Error: 'ids' must include at least two memory IDs when action is 'merge'.");
            }
            ...
            if (sourceMemories.some((memory) => memory.projectPath !== projectIdentity)) {
               return err("Error: All memories to merge must belong to the current project.");
            }
            ...
            const requestedCategoryTyped: MemoryCategory | undefined =
               requestedCategory && isMemoryCategory(requestedCategory) ? requestedCategory : undefined;
            ...
            if (!requestedCategoryTyped && sourceMemories.some((memory) => memory.category !== category)) {
               return err("Error: Mixed-category merges require an explicit 'category'.");
            }
            ...
            // already-merged memories. Mirrors Host's parity construction
            // at core/tools/ctx-memory/tools.ts:381-405.
            ...
            if (canonicalExisting) {
               ...
               if (canonicalMemory.content !== content || canonicalMemory.normalizedHash !== normalizedHash) {
