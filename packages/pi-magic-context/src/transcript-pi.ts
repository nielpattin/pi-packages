 ...
 * Unlike Host where `Part.text` is mutated in place, Pi messages
 ...
 * ## Shape normalization
 *
 * The transform pipeline is Host-shaped: it expects user messages
 * to contain tool_result parts (because Host folds tool results
...
import type { Transcript, TranscriptMessage, TranscriptPart, TranscriptPartKind } from "#core/shared/transcript";
...
export function createPiTranscript(
   ...
   const transcriptMessages: TranscriptMessage[] = buildTranscriptView(working, sessionId, (messageIndex) => {
...
function buildTranscriptView(
   ...
   while (i < working.length) {
      ...
      if (msg.role === "toolResult") {
         ...
         if (next?.role === "user") {
            result.push(createUserTranscriptMessage(working, i, sessionId, toolResultRun, markDirty));
            i += 1;
         } else {
            result.push(createSyntheticToolResultUserMessage(working, sessionId, toolResultRun, markDirty));
         }
         continue;
      }

      if (msg.role === "user") {
         result.push(createUserTranscriptMessage(working, i, sessionId, [], markDirty));
         i += 1;
         continue;
      }

      if (msg.role === "assistant") {
         result.push(createAssistantTranscriptMessage(working, i, sessionId, markDirty));
         i += 1;
         continue;
      }
      ...
      result.push(createOpaqueTranscriptMessage(working, i, sessionId, markDirty));
      i += 1;
   }
...
function createPiToolResultPart(
   ...
   const part = msg.content[partIndex];
   const kind: TranscriptPartKind = part?.type === "image" ? "image" : "tool_result";
   return {
      kind,
      id: msg.toolCallId,
      getText(): string | undefined {
 ...
 */
function extractStableId(msg: PiAgentMessage | undefined, index: number): string | undefined {
   if (msg === undefined) return undefined;
