 ...
 * singleton, but Desktop Host can load the same plugin once per project
 ...
 */
export async function startDreamScheduleTimer(args: ProjectRegistration): Promise<(() => void) | undefined> {
   ...
   if (!activeTimer) {
      ...
      log(`[dreamer] started independent schedule timer (every ${DREAM_TIMER_INTERVAL_MS / 60_000}m)`);
