...
- Found a project's source code path after searching → \`ctx_memory(action="write", category="ENVIRONMENT", content="Host source is at ~/Work/OSS/host")\`
...
- Can't remember where a related codebase or dependency lives → \`ctx_search(query="host source code path")\`
