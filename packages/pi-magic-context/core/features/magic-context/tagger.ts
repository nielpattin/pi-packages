...
function isAssignmentRow(row: unknown): row is AssignmentRow {
   ...
   if (candidate.type !== "message" && candidate.type !== "tool" && candidate.type !== "file") return false;
   if (candidate.tool_owner_message_id !== null && typeof candidate.tool_owner_message_id !== "string") return false;
   return true;
}
 ...
 * a session is created by exactly one harness (Host or Pi) and that origin
...
export function createTagger(): Tagger {
   ...
   function isUniqueConstraintError(error: unknown): boolean {
      return (
         error instanceof Error && "code" in error && (error as { code?: unknown }).code === "SQLITE_CONSTRAINT_UNIQUE"
      );
   }
   ...
   function getTag(sessionId: string, messageId: string, _type: NonToolTagType): number | undefined {
   ...
   function bindToolTag(sessionId: string, callId: string, ownerMsgId: string, tagNumber: number): void {
   ...
   function probeSignature(db: Database): { dataVersion: number; totalChanges: number } {
      const dvRow = getProbeDataVersionStatement(db).get() as { data_version: number } | null | undefined;
   ...
   function initFromDb(sessionId: string, db: Database): void {
      ...
      const row = db.prepare(GET_COUNTER_SQL).get(sessionId) as { counter: number } | null | undefined;
