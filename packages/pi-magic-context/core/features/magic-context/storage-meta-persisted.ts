...
function parseJsonArray<T>(json: string | null | undefined, validator: (value: unknown) => value is T): T[] {
   if (!json) return [];
   try {
...
export function setPersistedReasoningWatermark(db: Database, sessionId: string, tagNumber: number): void {
...
export function setPersistedStickyTurnReminder(db: Database, sessionId: string, text: string, messageId = ""): void {
   db.transaction(() => {
      ensureSessionMetaRow(db, sessionId);
      db.prepare(
...
export function setPersistedNoteNudgeTrigger(db: Database, sessionId: string, triggerMessageId = ""): void {
   db.transaction(() => {
      ensureSessionMetaRow(db, sessionId);
      db.prepare(
...
export function setPersistedNoteNudgeTriggerMessageId(db: Database, sessionId: string, triggerMessageId: string): void {
   db.transaction(() => {
      ensureSessionMetaRow(db, sessionId);
...
export function setPersistedDeliveredNoteNudge(db: Database, sessionId: string, text: string, messageId = ""): void {
   db.transaction(() => {
      ensureSessionMetaRow(db, sessionId);
      db.prepare(
...
export function getNoteNudgeAnchors(db: Database, sessionId: string): NoteNudgeAnchor[] {
   const row = db.prepare("SELECT note_nudge_anchors FROM session_meta WHERE session_id = ?").get(sessionId) as
      | { note_nudge_anchors?: string | null }
      | undefined;
   return parseJsonArray(row?.note_nudge_anchors, isValidNoteNudgeAnchor);
}

export function getAutoSearchHintDecisions(db: Database, sessionId: string): AutoSearchHintDecision[] {
   const row = db.prepare("SELECT auto_search_hint_decisions FROM session_meta WHERE session_id = ?").get(sessionId) as
...
function casUpdateJsonArrayColumn<T>(
   ...
   for (let attempt = 0; attempt < CAS_RETRY_LIMIT; attempt += 1) {
      const row = db.prepare(`SELECT ${column} FROM session_meta WHERE session_id = ?`).get(sessionId) as
...
export function appendNoteNudgeAnchor(db: Database, sessionId: string, messageId: string, text: string): boolean {
   if (!messageId || !text) return false;
   return casUpdateJsonArrayColumn(db, sessionId, "note_nudge_anchors", isValidNoteNudgeAnchor, (current) => {
...
export function pruneNoteNudgeAnchors(db: Database, sessionId: string, visibleMessageIds: Set<string>): number {
   let pruned = 0;
...
export function pruneAutoSearchHintDecisions(db: Database, sessionId: string, visibleMessageIds: Set<string>): number {
   let pruned = 0;
...
export function removeNoteNudgeAnchorByMessageId(db: Database, sessionId: string, messageId: string): boolean {
   let removed = false;
...
export function removeAutoSearchHintDecisionByMessageId(db: Database, sessionId: string, messageId: string): boolean {
   let removed = false;
   const ok = casUpdateJsonArrayColumn(
      db,
      sessionId,
...
export function getOverflowState(db: Database, sessionId: string): PersistedOverflowState {
   const result = db
      .prepare("SELECT detected_context_limit, needs_emergency_recovery FROM session_meta WHERE session_id = ?")
      .get(sessionId) as { detected_context_limit?: number; needs_emergency_recovery?: number } | undefined;
   if (!result) {
 ...
 */
export function recordOverflowDetected(db: Database, sessionId: string, reportedLimit: number | undefined): void {
   db.transaction(() => {
      ensureSessionMetaRow(db, sessionId);
 ...
 */
export function recordDetectedContextLimit(db: Database, sessionId: string, reportedLimit: number): void {
...
export function setPersistedCompactionMarkerState(
   ...
   db.prepare("UPDATE session_meta SET compaction_marker_state = ? WHERE session_id = ?").run(json, sessionId);
}

// ── Stripped placeholder message IDs ──

export function getStrippedPlaceholderIds(db: Database, sessionId: string): Set<string> {
   const row = db.prepare("SELECT stripped_placeholder_ids FROM session_meta WHERE session_id = ?").get(sessionId) as {
...
export function setStrippedPlaceholderIds(db: Database, sessionId: string, ids: Set<string>): void {
   ...
   db.prepare("UPDATE session_meta SET stripped_placeholder_ids = ? WHERE session_id = ?").run(json, sessionId);
}

export function removeStrippedPlaceholderId(db: Database, sessionId: string, messageId: string): boolean {
 ...
 * present (raw Host message + compartment row), then write
...
export interface PendingCompactionMarker {
   ...
   ordinal: number;
   /** Host message ID at the boundary (the user message just before the marker). */
   endMessageId: string;
...
export function getPendingCompactionMarkerState(db: Database, sessionId: string): PendingCompactionMarker | null {
   const row = db
...
export function getPendingPiCompactionMarkerState(db: Database, sessionId: string): PendingPiCompactionMarker | null {
   const row = db
...
export function peekDeferredExecutePending(db: Database, sessionId: string): DeferredExecutePayload | null {
   const row = db.prepare("SELECT deferred_execute_state FROM session_meta WHERE session_id = ?").get(sessionId) as {
