 ...
 */
export function updateTagByteSize(db: Database, sessionId: string, tagNumber: number, newByteSize: number): void {
...
export function updateTagStatus(db: Database, sessionId: string, tagId: number, status: TagEntry["status"]): void {
 ...
 */
export function updateCavemanDepth(db: Database, sessionId: string, tagNumber: number, depth: number): void {
...
export function updateTagMessageId(db: Database, sessionId: string, tagId: number, messageId: string): void {
 ...
 */
export function deleteTagsByMessageId(db: Database, sessionId: string, messageId: string): number[] {
...
function getOwnerScopedToolTagNumbers(db: Database, sessionId: string, ownerMsgId: string): number[] {
 ...
 */
export function getTagNumberByMessageId(db: Database, sessionId: string, messageId: string): number | null {
...
export function getTagsBySession(db: Database, sessionId: string): TagEntry[] {
   const rows = db
      .prepare(`SELECT ${TAG_SELECT_COLUMNS} FROM tags WHERE session_id = ? ORDER BY tag_number ASC, id ASC`)
      .all(sessionId)
      .filter(isTagRow);
 ...
 */
export function getTagsByNumbers(db: Database, sessionId: string, tagNumbers: readonly number[]): TagEntry[] {
...
// Host's per-turn callID counter could replay drop status onto fresh
 ...
 * for every candidate against the fallback session DB (via
 * `getMessageTimesFromFallbackDb`) and picks the most recent one whose
 * `time_created` precedes the current result message.
 *
 * The picking logic lives in the caller because resolving message times
 * requires the Host read-only DB handle, which lives in the hooks
...
/**
 * Pick the most recent (by Host `time_created`) candidate owner
 ...
 * `times` map (typically via `getMessageTimesFromFallbackDb`) and passes
 * it in. Splitting the lookup from the picking keeps `storage-tags.ts`
 * free of any Host-DB import.
 */
 ...
 * + `getMessageTimesFromFallbackDb` + `pickNearestPriorOwner` directly.
 *
