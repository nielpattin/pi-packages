...
import { clearStaleEntries, dequeueNext, getEntryRetryCount, removeDreamEntry, resetDreamEntry } from "./queue";
...
async function getActiveProjectSessionIds(args: {
   ...
   return args.db
      .prepare("SELECT session_id AS sessionId FROM session_meta WHERE is_subagent = 0 ORDER BY session_id ASC")
      .all()
...
async function identifyKeyFilesForSession(args: {
   ...
   log(`[key-files][${args.sessionId}] skipped because Pi-native read-history candidates are not implemented`);
}
...
export async function runDream(args: {
   ...
   const lastDreamAt =
      getDreamState(args.db, `last_dream_at:${args.projectIdentity}`) ?? getDreamState(args.db, "last_dream_at");
   ...
   try {
      for (const taskName of args.tasks) {
         ...
         const leaseRenewalInterval = setInterval(() => {
            try {
               ...
               log(`[dreamer] task ${taskName}: lease renewal threw — aborting LLM call: ${getErrorMessage(err)}`);
      ...
      if (!circuitBreakerTripped && !lostLease && args.experimentalPinKeyFiles?.enabled && Date.now() <= deadline) {
         ...
         try {
            ...
            log("[key-files] skipped because Pi-native read-history candidates are not implemented");
...
export async function processDreamQueue(args: {
    ...
    * to this project are dequeued. Each running Host/Pi process registers
