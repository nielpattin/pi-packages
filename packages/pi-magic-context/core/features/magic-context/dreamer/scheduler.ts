...
export function parseScheduleWindow(schedule: string): { startMinutes: number; endMinutes: number } | null {
 ...
 *   Each running Host/Pi process registers exactly one project, so it
 ...
 *   `host-anthropic-auth` ends up trying to spawn `pi --print` for
 *   `host-xtra` (a project Pi was never opened in), failing every cycle.
 *
 ...
 */
export function checkScheduleAndEnqueue(db: Database, schedule: string, ownProjectIdentity?: string): number {
