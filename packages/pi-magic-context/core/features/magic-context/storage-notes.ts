...
export function getNotes(db: Database, options: GetNotesOptions = {}): Note[] {
   ...
   return (db.prepare(`SELECT * FROM notes${where} ORDER BY created_at ASC, id ASC`).all(...params) as unknown[])
      .filter(isNoteRow)
      .map(toNote);
}
...
export function addNote(db: Database, type: NoteType, options: SessionNoteInput | SmartNoteInput): Note {
   const now = Date.now();
   const result =
      type === "session"
         ? db
...
export function dismissNote(db: Database, noteId: number, scope: NoteMutationScope): boolean {
   ...
   const result = db
      .prepare("UPDATE notes SET status = 'dismissed', updated_at = ? WHERE id = ? AND status != 'dismissed'")
      .run(Date.now(), noteId);
