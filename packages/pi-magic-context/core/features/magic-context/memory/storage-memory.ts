...
import type { Memory, MemoryCategory, MemoryInput, MemorySourceType, MemoryStatus, VerificationStatus } from "./types";
...
export function updateMemoryVerification(db: Database, id: number, verificationStatus: VerificationStatus): void {
   const now = Date.now();
   getUpdateMemoryVerificationStatement(db).run(verificationStatus, verificationStatus, now, now, id);
}

export function updateMemoryContent(db: Database, id: number, content: string, normalizedHash: string): void {
