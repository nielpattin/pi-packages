...
import { getLegacyHostMagicContextStorageDir, getMagicContextStorageDir } from "../../shared/data-path";
...
/**
 * One-time migration of legacy plugin data into the shared
 * ~/.pi/agent/pi-magic-context location. Runs lazily on first openDatabase() call.
 *
 * Safety guarantees:
...
function migrateLegacyStorageIfNeeded(targetDbPath: string, targetDbDir: string): void {
   ...
   const legacyDir = getLegacyHostMagicContextStorageDir();
   ...
   // -wal; if the Host plugin was running when the user upgraded, real
...
export function initializeDatabase(db: Database): void {
   ...
   db.exec(`
    CREATE TABLE IF NOT EXISTS tags (
      ...
      byte_size INTEGER,
      tag_number INTEGER,
      harness TEXT NOT NULL DEFAULT 'host',
...
CREATE INDEX IF NOT EXISTS idx_dream_queue_pending ON dream_queue(started_at, enqueued_at);
  ...
  `);
   ...
   // existing rows with the default, so Host users transparently get
   // harness='host' on all pre-existing data. Pi will be added later
   // by its own plugin entry, also writing to the same shared DB.
   //
   // We don't (yet) include harness in WHERE clauses — Host session IDs
   ...
   ensureColumn(db, "tags", "harness", "TEXT NOT NULL DEFAULT 'host'");
   ensureColumn(db, "pending_ops", "harness", "TEXT NOT NULL DEFAULT 'host'");
   ensureColumn(db, "source_contents", "harness", "TEXT NOT NULL DEFAULT 'host'");
   ensureColumn(db, "compartments", "harness", "TEXT NOT NULL DEFAULT 'host'");
   ensureColumn(db, "compression_depth", "harness", "TEXT NOT NULL DEFAULT 'host'");
   ensureColumn(db, "session_facts", "harness", "TEXT NOT NULL DEFAULT 'host'");
   ensureColumn(db, "session_meta", "harness", "TEXT NOT NULL DEFAULT 'host'");
   ensureColumn(db, "recomp_compartments", "harness", "TEXT NOT NULL DEFAULT 'host'");
   ensureColumn(db, "recomp_facts", "harness", "TEXT NOT NULL DEFAULT 'host'");
   ensureColumn(db, "message_history_index", "harness", "TEXT NOT NULL DEFAULT 'host'");
 ...
 *      tries to drive transforms. On Pi/Host this can let the full
