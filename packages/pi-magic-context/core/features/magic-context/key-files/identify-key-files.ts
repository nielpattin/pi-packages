 ...
 */
export function buildKeyFilesPrompt(candidates: FileReadStat[], tokenBudget: number, minReads: number): string {
...
/**
 * Get candidate files for key-file analysis from Host's DB.
...
export function computeGenerationConfigHash(config: Pick<V6KeyFilesConfig, "token_budget" | "min_reads">): string {
...
function buildV6KeyFilesPrompt(args: {
   ...
   const current = args.currentRows
      .map((row) => `- ${row.path}: ${row.localTokenEstimate} tokens, stale=${row.staleReason ?? "fresh"}`)
      .join("\n");
...
export function validateLlmOutput(raw: string, config: V6KeyFilesConfig, projectPath: string): ValidatedKeyFilesOutput {
   let obj: unknown;
   try {
   ...
   if (typeof record.no_change !== "boolean") throw new KeyFilesValidationError("missing no_change");
   if (!Array.isArray(record.files)) throw new KeyFilesValidationError("missing files array");
   if (record.no_change && record.files.length > 0) throw new KeyFilesValidationError("no_change=true with files");
   ...
   for (const item of record.files) {
      ...
      if ("source_files" in file) {
         throw new KeyFilesValidationError(`source_files field not allowed (one file per row): ${String(file.path)}`);
      }
      if (typeof file.path !== "string" || file.path.length === 0) throw new KeyFilesValidationError("bad path");
      ...
      if (seenLower.has(file.path.toLowerCase())) throw new KeyFilesValidationError(`case-dup: ${file.path}`);
      seen.add(file.path);
      seenLower.add(file.path.toLowerCase());
      if (!isRelativeProjectFile(projectPath, file.path)) throw new KeyFilesValidationError(`unreadable: ${file.path}`);
      if (typeof file.content !== "string") throw new KeyFilesValidationError(`bad content: ${file.path}`);
      if (file.content.length > 100_000) throw new KeyFilesValidationError(`content >100KB: ${file.path}`);
   ...
   if (total > config.token_budget) throw new KeyFilesValidationError(`total ${total} > budget ${config.token_budget}`);
...
export async function runKeyFilesTask(args: {
   ...
   try {
      ...
      if (validated.no_change) return { committedVersion: null, candidates: candidates.length, noChange: true };
