...
/**
 * Query file read patterns from Host's DB for a specific session.
 * Returns files that were fully read (no line range) at least `minReads` times,
 * sorted by read frequency descending.
 */
export function getSessionReadStats(hostDb: Database, sessionId: string, minReads: number): FileReadStat[] {
   // Step 1: Get all full-read file paths with counts and latest output size
   const fullReads = hostDb
      .prepare(
         `
        WITH full_reads AS (
            SELECT 
