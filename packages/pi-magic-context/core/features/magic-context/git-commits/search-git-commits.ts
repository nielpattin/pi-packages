...
export function searchGitCommitsSync(
   ...
   if (sanitized.length > 0) {
      try {
         for (const row of getFtsStatement(db).all(projectPath, sanitized, fetchLimit) as CommitRow[]) {
   ...
   if (ftsCandidates.length === 0) {
      for (const row of getLikeFallbackStatement(db).all(projectPath, trimmed, fetchLimit) as CommitRow[]) {
         ftsCandidates.push(rowToCommit(row));
      }
   }
