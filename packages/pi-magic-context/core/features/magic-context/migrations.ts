...
const MIGRATIONS: Migration[] = [
   {
      ...
      up: (db: Database) => {
         ...
         if (hasSessionNotes) {
            const sourceCount = (db.prepare("SELECT COUNT(*) as c FROM session_notes").get() as { c: number }).c;
            const migratedCount = (
         ...
         if (hasSmartNotes) {
            const sourceCount = (db.prepare("SELECT COUNT(*) as c FROM smart_notes").get() as { c: number }).c;
            const migratedCount = (
   ...
   },
   {
      ...
      // plugin v0.16+ we share `~/.pi/agent/pi-magic-context/`
      // between Host and Pi, so every session-scoped table needs to
      ...
      // so all pre-v0.16 notes transparently become harness='host'.
      up: (db: Database) => {
   ...
   },
   {
      ...
      // for the combined per-pass workload).
      //
   ...
   },
   {
      ...
      // keyed solely by (session_id, callID), but Host generates
      ...
      // separate backfill pass populates owner from the fallback session DB
 ...
 * same `schema_migrations` row we're about to insert. Two Host/Pi
