...
function inferToastVariant(text: string): "success" | "error" | "warning" | "info" {
   ...
   if (lower.includes("error") || lower.includes("failed") || lower.includes("alert")) return "error";
 ...
 */
export async function sendUserPrompt(client: unknown, sessionId: string, text: string): Promise<void> {
   if (!hasNotificationSessionClient(client)) {
