...
export function isReduceToolPart(part: unknown): boolean {
   ...
   // Host format: { type: "tool", tool: "ctx_reduce" }
   if (part.type === "tool" && typeof part.tool === "string" && STALE_TOOL_NAMES.has(part.tool)) return true;
   ...
   if (part.type === "tool_use" && typeof part.name === "string" && STALE_TOOL_NAMES.has(part.name)) return true;
   return false;
}
...
function hasAnyMeaningfulPart(parts: unknown[]): boolean {
   for (const part of parts) {
      ...
      if (part.type === "thinking" || part.type === "reasoning" || part.type === "redacted_thinking") continue;
