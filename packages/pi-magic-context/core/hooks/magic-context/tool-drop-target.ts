...
const IGNORE_PART_TYPES = new Set(["thinking", "reasoning", "redacted_thinking", "meta", "step-start", "step-finish"]);
...
function truncateToolPart(part: unknown): void {
   ...
   // Host format: { type: "tool", state: { input: {...}, output: "..." } }
   if (part.type === "tool" && isRecord(part.state)) {
   ...
   // Host invocation format: { type: "tool-invocation", args: {...} }
...
function truncateInputValues(input: Record<string, unknown>): void {
   for (const key of Object.keys(input)) {
      ...
      if (typeof value === "string") {
         ...
         if (value.endsWith(TRUNCATION_SENTINEL) || value === "[object]" || /^\[\d+ items\]$/.test(value)) continue;
