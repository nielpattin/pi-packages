...
import { cleanupHistorianStateFile, maybeWriteHistorianStateFile } from "./compartment-runner-incremental";
...
export async function executeContextRecompInternal(deps: CompartmentRunnerDeps): Promise<string> {
   ...
   try {
      ...
      let candidateFacts: Array<{ category: string; content: string }> = existingStaging?.facts ?? [];
      ...
      async function promoteAndFinalize(reason: string): Promise<string | null> {
         ...
         if (deps.directory && deps.memoryEnabled !== false && deps.autoPromote !== false) {
            ...
            promoteSessionFactsToMemory(db, sessionId, resolveProjectIdentity(deps.directory), promoted.facts);
         }
      ...
      while (offset < protectedTailStart) {
         ...
         // State file lives under <project>/.host/magic-context/historian/
         // so historian's Read tool doesn't trigger Host's
         ...
         candidateCompartments = [...candidateCompartments, ...(validatedPass.compartments ?? [])];
