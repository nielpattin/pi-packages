 ...
 * from Host's session DB via `withReadOnlySessionDb`. Other harnesses
 ...
 * precedence over the Host-DB default. Sessions never registered
 * here continue to read from Host's DB (existing behavior).
 *
 ...
 * unregister function. Pass-through harnesses (Host) never call
 ...
 */
export function withRawMessageProvider<T>(sessionId: string, provider: RawMessageProvider, fn: () => T): T {
...
export function getRawSessionMessageCount(sessionId: string): number {
   ...
   return withReadOnlySessionDb((db) => getRawSessionMessageCountFromDb(db, sessionId)) ?? 0;
}
...
export function getRawSessionTagKeysThrough(sessionId: string, upToMessageIndex: number): RawSessionTagKeys {
...
export function readSessionChunk(
   ...
   return {
      ...
      hasMore:
         lastOrdinal <
         (eligibleEndOrdinal !== undefined ? Math.min(eligibleEndOrdinal - 1, messages.length) : messages.length),
      text: lines.join("\n"),
      lines: lineMeta,
