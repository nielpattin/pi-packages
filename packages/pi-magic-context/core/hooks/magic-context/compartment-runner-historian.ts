...
// (<project>/.host/magic-context/historian/) so they sit inside the
// project boundary Host's permission system already trusts AND so users
   ...
   try {
      ...
      for (let retryIndex = 0; retryIndex <= MAX_HISTORIAN_RETRIES; retryIndex += 1) {
         try {
            await shared.promptSyncWithModelSuggestionRetry(
               client,
               {
                  ...
                  body: {
                     ...
                     // HISTORIAN_EDITOR_AGENT for two-pass editor pass) so Host
                     // loads the right system prompt. When modelOverride is set,
                     // Host uses the override model but still loads the agent's
                     // registered system prompt.
                     agent: agentId,
                     ...(modelOverride ? { model: modelOverride } : {}),
                     // synthetic: true keeps this big internal prompt out of the
                     // Host TUI subagent pane (would otherwise render as a huge
            ...
            shared.sessionLog(parentSessionId, `historian: prompt attempt ${retryIndex + 1} failed: ${errorMsg}`);
      ...
      if (agentSessionId) {
         await client.session.delete({ path: { id: agentSessionId } }).catch((e: unknown) => {
            shared.sessionLog(parentSessionId, "compartment agent: session cleanup failed", getErrorMessage(e));
         });
      }
   }
}
...
function dumpHistorianResponse(sessionId: string, directory: string, label: string, text: string): string | undefined {
   try {
