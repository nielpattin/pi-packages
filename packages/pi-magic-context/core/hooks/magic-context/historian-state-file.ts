 ...
 * (`<project>/.host/magic-context/historian/`) and the prompt instructs
 ...
 * `os.tmpdir()` because Host's `external_directory` permission system
 * asks the user before letting the historian subagent's Read tool open any
 * file outside the project. Anchoring under `<project>/.host/` keeps
 ...
 *
 * Shared between Host (`compartment-runner-incremental.ts`,
 ...
 * `<directory>/.host/magic-context/historian/`. The dir is created
 * recursively on first write, so the call is safe on fresh projects that have
 * never had a `.host/` subtree.
 */
