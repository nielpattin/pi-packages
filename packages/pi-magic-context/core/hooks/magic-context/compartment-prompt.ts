...
- Fact dedup examples:
  - These are DUPLICATES (merge into one): "Plugin config uses layered JSONC files" and "AFT plugin config uses layered JSONC files at ~/.config/host/aft.jsonc and <project>/.host/aft.jsonc, with project values deep-merging over user values." → keep the longer, more specific version only.
...
Input notes:
- [N] or [N-M] is a stable raw Host message range.
- U: means user.
- A: means assistant.
