 ...
 *
 * Mirrors Host's `ConfigVariable.substitute` semantics so users can share
 * the same patterns across `host.json(c)` and `magic-context.jsonc`:
 ...
 *
 * Unlike Host we treat missing values as warnings rather than hard errors:
 * magic-context config is less critical than the main Host config, and a
...
export function substituteConfigVariables(input: SubstituteInput): SubstituteResult {
   ...
   if (input.isProjectConfig) {
      ...
      if (hasEnvTokens || hasFileTokens) {
         const tokenTypes = [hasEnvTokens ? "{env:}" : undefined, hasFileTokens ? "{file:}" : undefined]
            .filter(Boolean)
            .join(" and ");
         warnings.push(
   ...
   for (const match of fileMatches) {
      ...
      // Skip tokens inside line comments: agents-only feature, matches Host.
