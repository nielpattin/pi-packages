...
export const DEFAULT_DREAMER_TASKS: DreamingTask[] = ["consolidate", "verify", "archive-stale", "improve"];
 ...
 *  Pi-only — Host uses `variant` in agent config instead. */
export const PiThinkingLevelSchema = z.enum(["off", "minimal", "low", "medium", "high", "xhigh"]).optional();
 ...
 *  enabled via Host variant config. */
...
export interface MagicContextConfig {
   enabled: boolean;
   /** Auto-update the cached Host plugin wrapper when a newer npm version is available.
    ...
    *
    * Internal Host hidden agents (title, summary, compaction) are
...
export const MagicContextConfigSchema = z
   .object({
      ...
      enabled: z.boolean().default(true),
      /** Enable automatic npm self-update checks for the Host plugin.
      ...
      cache_ttl: z.union([z.string(), z.object({ default: z.string() }).catchall(z.string())]).default("5m"),
      ...
      history_budget_percentage: z.number().min(0.05).max(0.5).default(DEFAULT_HISTORY_BUDGET_PERCENTAGE),
       ...
       *  blocks (issue #53). Host's internal hidden agents — title,
      ...
      compressor: z
         .object({
            ...
            min_compartment_ratio: z.number().min(100).max(10000).default(DEFAULT_COMPRESSOR_MIN_COMPARTMENT_RATIO),
            ...
            max_merge_depth: z.number().min(1).max(5).default(DEFAULT_COMPRESSOR_MAX_MERGE_DEPTH),
            ...
            max_compartments_per_pass: z.number().min(3).max(50).default(DEFAULT_COMPRESSOR_MAX_COMPARTMENTS_PER_PASS),
            /** Number of newest compartments always excluded from compression.
