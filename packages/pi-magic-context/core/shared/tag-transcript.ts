 ...
 *
 * This is a deliberately minimal alternative to the Host-specific
 * `tag-messages.ts` that operates on `MessageLike[]`. The Host flow
 ...
 *
 * Most of that is Host-specific (cache stability across multi-pass
 ...
 * occurrences (mirrors Host tag-messages.ts:196-220). When a drop
 ...
 *
 * Reuses unchanged from the Host path:
 *
...
import { byteSize, prependTag, stripTagPrefix } from "../hooks/magic-context/tag-content-primitives";
...
export function tagTranscript(
   ...
   // atomicity we need. Removing the wrapper matches Host's
      ...
      for (let partIndex = 0; partIndex < parts.length; partIndex += 1) {
         ...
         if (part.kind === "tool_use" || part.kind === "tool_result") {
            ...
            if (existing) {
               ...
               if (existing.inputByteSize === 0 && part.kind === "tool_use" && meta.inputByteSize > 0) {
               ...
               // (matches Host behavior — only result gets the prefix).
               ...
               targets.set(existing.tagId, buildAggregateTarget(existing.tagId, existing.occurrences));
            } else {
               ...
               // when it's a tool_result. (Host parity: prefix
...
function tagTextPart(args: TagTextPartArgs): void {
   ...
   const tagId = args.tagger.assignTag(args.sessionId, contentId, "message", byteSize(text), args.db);
...
function tagToolPart(args: TagToolPartArgs): void {
   // Prefer the part's stable id (tool call id from Pi/Host); fall
   ...
   // matches the Host behavior of tagging tool outputs.
 ...
 *
 * Mirrors Host's createToolDropTarget semantics in tool-drop-target.ts.
 */
function buildAggregateTarget(tagId: number, occurrences: ToolOccurrence[]): TagTarget {
   ...
   return {
      ...
      truncate(): "truncated" | "absent" {
         ...
         // matches Host's truncate sentinel exactly.
 ...
 */
function buildTextTarget(part: TranscriptPart, message: { info: { id?: string; role: string } }): TagTarget {
   return {
      setContent(content: string): boolean {
 ...
 */
function buildToolTarget(part: TranscriptPart, message: { info: { id?: string; role: string } }): TagTarget {
   return {
      setContent(content: string): boolean {
