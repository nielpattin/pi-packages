 ...
 * each as a child "session" with its own model/prompt/tools. Host and Pi
 * have very different APIs for this:
 *
 *   - Host: `client.session.create({parentID}) → client.session.prompt() →
 *     client.session.messages() → client.session.delete()`. The plugin runs
 *     in-process with the Host server and uses its SDK client directly.
 *
 ...
 * Step 5b will refactor the Host-side spawn paths in
 * `compartment-runner-historian.ts`, `dreamer/runner.ts`, and
 * `sidekick/agent.ts` onto an `HostSubagentRunner` so both harnesses
 * share the agent business logic instead of duplicating it. Until 5b lands,
 * Host keeps its existing direct `client.session.*` calls untouched —
 * the runner contract is purely additive on the Host side.
 */

/**
 * Configuration for one subagent invocation.
 *
 * Mirrors the union of Host's `session.create` + `session.prompt` body
 ...
 *
 * Fields:
 * - `agent`: harness-specific agent name. Host looks this up in its
 ...
 *   on the Pi side and used only by `HostSubagentRunner`.
 ...
 * - `cwd`: working directory for the child. Host uses this for
...
export interface SubagentRunOptions {
    ...
    * Pi subprocess. Host ignores this field — thinking/reasoning is
    * controlled via `variant` in the Host agent config instead.
    *
 ...
 *   message_end with terminal stopReason and no toolCall; Host: SDK
 *   `agent_end` equivalent).
 * - `raw_event` — every parsed event from the harness's structured output
 *   stream (Pi NDJSON / Host SDK events). Emitted unconditionally so
 ...
 *   here so the Host runner can surface the child session ID for log
 ...
 *
 * Each harness ships a single instance — the Host plugin wires
 * `HostSubagentRunner` and the Pi plugin wires `PiSubagentRunner` in
...
export interface SubagentRunner {
   /** Human-readable harness name, for logging (`"host"` or `"pi"`). */
