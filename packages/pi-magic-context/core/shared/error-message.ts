 ...
 * carry the actual signal (e.g. `NotFoundError` with no message on Host
...
export function describeError(error: unknown): ErrorDescription {
   ...
   if (cause && typeof cause === "object") {
      ...
      causeName = readString(causeRecord.name) ?? (cause as { constructor?: { name?: string } }).constructor?.name;
   }
