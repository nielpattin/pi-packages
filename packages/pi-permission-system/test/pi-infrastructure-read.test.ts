...
describe("discoverGlobalNodeModulesRoot", () => {
   ...
   test("the discovered path includes the pi-permission-system package directory", () => {
      ...
      expect(`${root!}/pi-permission-system`).toBe("/opt/homebrew/lib/node_modules/pi-permission-system");
   });
});
...
describe("isPiInfrastructureRead", () => {
   ...
   test("allows 'read' tool for a file inside agentDir", () => {
      expect(isPiInfrastructureRead("read", "/home/user/.pi/agent/permission.jsonc", INFRA_DIRS, CWD)).toBe(true);
   });
   ...
   test("blocks 'write' tool for a file inside agentDir", () => {
      expect(isPiInfrastructureRead("write", "/home/user/.pi/agent/permission.jsonc", INFRA_DIRS, CWD)).toBe(false);
   });
