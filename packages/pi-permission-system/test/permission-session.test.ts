...
const { mockGetActiveAgentName, mockGetActiveAgentNameFromSystemPrompt, mockCreatePermissionManagerForCwd } =
   vi.hoisted(() => ({
...
describe("PermissionSession", () => {
   describe("constructor and delegation", () => {
      it("delegates checkPermission to internal PermissionManager", () => {
         ...
         expect(pm.checkPermission).toHaveBeenCalledWith("bash", { command: "ls" }, undefined, undefined);
   ...
   describe("infrastructure paths", () => {
      it("getInfrastructureDirs returns paths from ExtensionPaths", () => {
         const { session } = createSession();
         expect(session.getInfrastructureDirs()).toEqual(["/test/agent", "/test/agent/git"]);
      });
   ...
   describe("canPrompt", () => {
      ...
      it("returns false when runtimeDeps says no", () => {
         const runtimeDeps = makeRuntimeDeps();
         (runtimeDeps.canRequestPermissionConfirmation as ReturnType<typeof vi.fn>).mockReturnValue(false);
         const { session } = createSession({ runtimeDeps });
