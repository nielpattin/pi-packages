...
test("buildResolvedConfigLogEntry includes policy paths and legacy detection flags", () => {
   const policyPaths: ResolvedPolicyPaths = {
      globalConfigPath: "/home/user/.pi/agent/permission.jsonc",
   ...
   expect(result.globalConfigPath).toBe("/home/user/.pi/agent/permission.jsonc");
...
test("config.resolved entry appears in review log via logger", () => {
   ...
   try {
      ...
      const globalConfigPath = join(tempDir, "permission.jsonc");
