 ...
 * "allow" by default.
 */
function makeStatefulSession(overrides: Partial<Record<keyof PermissionSession, unknown>> = {}): PermissionSession {
   const sessionRules: Rule[] = [];

   const checkPermission = vi
      .fn()
      .mockImplementation(
         (surface: string, input: unknown, _agentName?: string, rules?: Rule[]): PermissionCheckResult => {
