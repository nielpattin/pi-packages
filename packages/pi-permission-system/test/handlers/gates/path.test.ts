...
describe("describePathGate", () => {
   ...
   it("returns null when tool has no extractable path", () => {
      const checkPermission = vi.fn<CheckPermissionFn>();
      const result = describePathGate(makeTcc({ toolName: "read", input: {} }), checkPermission, getSessionRuleset);
      expect(result).toBeNull();
   });

   it("returns null when path check result is allow", () => {
      const checkPermission = vi.fn<CheckPermissionFn>().mockReturnValue(makeCheckResult({ state: "allow" }));
   ...
   it("descriptor denialContext references the file path and tool name", () => {
      ...
      const result = describePathGate(makeTcc(), checkPermission, getSessionRuleset) as GateDescriptor;
   ...
   it("passes agentName and session rules to checkPermission", () => {
      ...
      expect(checkPermission).toHaveBeenCalledWith("path", { path: ".env" }, "my-agent", sessionRules);
   });
});
