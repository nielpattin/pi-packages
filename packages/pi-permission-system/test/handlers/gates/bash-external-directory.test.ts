...
describe("describeBashExternalDirectoryGate", () => {
   ...
   it("returns GateBypass when all external paths are session-covered", async () => {
      const checkPermission = vi.fn().mockReturnValue(makeCheckResult("allow", { source: "session" }));
      const result = await describeBashExternalDirectoryGate(makeTcc(), checkPermission, vi.fn().mockReturnValue([]));
   ...
   it("returns GateBypass when all external paths are config-level allowed", async () => {
      ...
      const checkPermission = vi.fn().mockImplementation((_surface: string, input: Record<string, unknown>) => {
