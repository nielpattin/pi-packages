...
export async function waitForForwardedPermissionApproval(
   ...
   const location = ensurePermissionForwardingLocation(deps.logger, deps.forwardingDir, targetSessionId);
   if (!location) {
      logPermissionForwardingError(
         deps.logger,
   ...
   try {
      ...
      logPermissionForwardingError(deps.logger, `Failed to write forwarded permission request '${requestPath}'`, error);
      return { approved: false, state: "denied" };
   }
