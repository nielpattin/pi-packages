...
import { formatMissingToolNameReason, formatSkillAskPrompt, formatUnknownToolReason } from "#src/permission-prompts";
...
export class PermissionGateHandler {
   ...
   async handleToolCall(event: unknown, ctx: ExtensionContext): Promise<{ block?: true; reason?: string }> {
      ...
      const promptPermission = (details: PromptPermissionDetails) => this.session.prompt(ctx, details);
      ...
      const checkPermission: GateRunnerDeps["checkPermission"] = (surface, input, agent, sessionRules) =>
      ...
      if (skillDescriptor) {
         const skillResult = await runGateCheck(skillDescriptor, tcc.agentName, tcc.toolCallId, runnerDeps);
      ...
      const infraDirs = [...this.session.getInfrastructureDirs(), ...this.session.getInfrastructureReadPaths()];
      ...
      if (extDirDesc) {
         if (isGateBypass(extDirDesc)) {
            ...
            const extDirResult = await runGateCheck(extDirDesc, tcc.agentName, tcc.toolCallId, runnerDeps);
      ...
      const bashExtDesc = await describeBashExternalDirectoryGate(tcc, checkPermission, getSessionRuleset);
      if (bashExtDesc) {
         if (isGateBypass(bashExtDesc)) {
            ...
            const bashExtResult = await runGateCheck(bashExtDesc, tcc.agentName, tcc.toolCallId, runnerDeps);
      ...
      if (bashPathDesc) {
         if (isGateBypass(bashPathDesc)) {
            ...
            const bashPathResult = await runGateCheck(bashPathDesc, tcc.agentName, tcc.toolCallId, runnerDeps);
