...
import { isPathOutsideWorkingDirectory, normalizePathForComparison } from "#src/path-utils";
 ...
 */
function collectPatternCommandTokens(node: TSNode, tokens: string[], config: PatternCommandConfig): void {
...
export async function extractExternalPathsFromBashCommand(command: string, cwd: string): Promise<string[]> {
   ...
   for (const token of tokens) {
      ...
      if (isPathOutsideWorkingDirectory(candidate, cwd) && !seen.has(normalized)) {
         seen.add(normalized);
