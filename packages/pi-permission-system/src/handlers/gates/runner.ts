...
export async function runGateCheck(
   ...
   const hasSessionApproval = gateResult.action === "allow" && gateResult.sessionApproval !== undefined;
   ...
   deps.emitDecision({
      ...
      resolution: deriveResolution(check.state, gateResult.action, hasSessionApproval, canConfirm, autoApproved),
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- ?? null normalises undefined to null for the log record
      origin: check.origin ?? null,
      agentName: agentName ?? null,
      matchedPattern: check.matchedPattern ?? null,
   ...
   if (gateResult.action === "allow" && hasSessionApproval) {
      if (descriptor.sessionApproval) {
         if ("patterns" in descriptor.sessionApproval) {
            ...
            deps.approveSessionRule(descriptor.sessionApproval.surface, descriptor.sessionApproval.pattern);
         }
      }
   }
