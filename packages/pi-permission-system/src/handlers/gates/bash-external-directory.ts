...
export async function describeBashExternalDirectoryGate(
   ...
   for (const p of externalPaths) {
      const check = checkPermission("external_directory", { path: p }, tcc.agentName ?? undefined, bashSessionRules);
   ...
   const worstCheck = uncoveredEntries.find(({ check }) => check.state === "deny")?.check ?? uncoveredEntries[0].check;
