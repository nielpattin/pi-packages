...
export function findSkillPathMatch(
   ...
   const comparablePath = normalizePathForComparison(normalizedPath, normalizedPath);

   for (const entry of entries) {
      if (entry.normalizedLocation && comparablePath === entry.normalizedLocation) {
         return entry;
      }
   }
   ...
   for (const entry of entries) {
      if (!entry.normalizedBaseDir || !isPathWithinDirectory(comparablePath, entry.normalizedBaseDir)) {
         continue;
      }
